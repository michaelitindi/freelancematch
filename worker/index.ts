...
import { createDBWrapper, generateId, jsonParse } from '../src/lib/db-wrapper';
import { generateToken, verifyToken, hashPassword, verifyPassword } from '../src/lib/auth';
...
// Mark event as read
app.patch('/events/:id/read', async (c) => {
  const db = c.get('db');
  const id = c.req.param('id');
  
  await db.prepare('UPDATE realtime_events SET is_read = 1 WHERE id = ?').run(id);
